...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon: Icon, rating, reviewCount }) => (
  <Card className="bg-card/70 backdrop-blur-sm shadow-md border-border/30 hover:border-primary/30 transition-all hover:shadow-primary/20"> {/* Removed h-full */}
    ...
    <CardContent className="pb-4 px-4">
      ...
      {rating !== undefined && rating !== null && reviewCount !== undefined && (
  ...
  return (
    <Card className="bg-card/80 backdrop-blur-lg border-border/40 shadow-lg"> {/* Removed h-full */}
        ...
        <CardContent className="grid gap-4 grid-cols-2">
            ...
            <StatCard title="Resources" value={formatNumberWithSuffix(stats.resourcesPublishedCount)} icon={Package} />
            <StatCard title="Reviews Written" value={formatNumberWithSuffix(stats.reviewsPublishedCount)} icon={MessageSquare} />
            <StatCard 
                title="Avg. Resource Rating" 
