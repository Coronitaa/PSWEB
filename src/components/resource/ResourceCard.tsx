...
export function ResourceCard({ resource, compact = false, onHoverChange, onOverflowHoverChange }: ResourceCardProps) {
  ...
  return (
    <div
      className={cn(
        "relative h-full group/card-container hover:z-40", 
      )}
              ...
              <div className="aspect-video overflow-hidden rounded-md mb-3 shadow-inner" onClick={(e) => e.stopPropagation()}>
                {hasGalleryForOverlay ? (
                  <NestedCarousel
                    ...
                    className="h-full" // Esto es importante para que el carrusel tome la altura del aspect-video
                  >
                    {imageGalleryForOverlay.map((imgUrl, idx) => (
                      <NestedCarouselItem key={idx} className="h-full"> {/* Asegura que el item tome la altura */}
                        <Image
                          src={imgUrl}
